/* รวมรอบ 7: เพิ่มข้อสอบเก็ง 2569 — English เงื่อนไข (p5_english) + คณิตหลายหัวข้อ (p5_math) */
const fs = require("fs");
...
const FREQ_TOPIC = {
  ...
  "คณิตศาสตร์: ร้อยละ": 4, "คณิตศาสตร์: กำไร-ขาดทุน": 4, "คณิตศาสตร์: อัตราส่วน": 3,
  "คณิตศาสตร์: ห.ร.ม./ค.ร.น.": 3, "คณิตศาสตร์: สมการ/โจทย์ปัญหา": 4,
  "คณิตศาสตร์: ดอกเบี้ย": 3, "คณิตศาสตร์: การปักเสา": 4, "คณิตศาสตร์: พื้นที่ผิวและปริมาตร": 4, "คณิตศาสตร์: จำนวนแรงงาน": 4,
...
/* ---- core: เดิม + p5 (predicted 2569) ---- */
let incoming = [];
["p5_english.json", "p5_math.json"].forEach((f) => { JSON.parse(fs.readFileSync(path.join(bd, f), "utf8")).forEach((q) => { q.predicted = true; q.year = 2569; incoming.push(q); }); });
...
/* ---- คลังเฉพาะทาง: คงเดิม (re-id ต่อท้าย) ---- */
const META = {
...
const out = `/* ============================================================
   ...
   สร้างอัตโนมัติจาก _batches/assemble7.js
   ============================================================ */
...
const bySub = {}, pred = core.filter((q) => q.predicted).length;
core.forEach((q) => { bySub[q.subject] = (bySub[q.subject] || 0) + 1; });
console.log("CORE:", core.length, "(เก็ง 2569:", pred + ")", JSON.stringify(bySub));
