/* รวมรอบ 6: เติมข้อสอบ core (c4_*) + ภาค ข (xb4_laws) คงของเดิมทั้งหมด + freq ใหม่ */
const fs = require("fs");
...
/* ---- 1) core (เดิม + c4_*) ---- */
let incoming = [];
["c4_analytical.json", "c4_ethics.json", "c4_english.json"].forEach((f) => { JSON.parse(fs.readFileSync(path.join(bd, f), "utf8")).forEach((q) => incoming.push(q)); });
...
/* ---- 2) คลังเฉพาะทาง (เดิม + xb4_laws แบบหลาย bank) ---- */
const META = {
...
const out = `/* ============================================================
   ...
   สร้างอัตโนมัติจาก _batches/assemble6.js
   ============================================================ */
