/* รวมรอบ 5: เติมข้อสอบ core + เก็ง 2569 + ภาค ข/ค (ไฟล์เติมแบบหลาย bank ในไฟล์เดียว)
   คงของเดิมทั้งหมด + เพิ่มใหม่ + คำนวณ freq ใหม่ทุกข้อ (ภาค ก/ข) */
const fs = require("fs");
...
/* ---- 1) core ---- */
let incoming = [];
["c3_analytical.json", "c3_ethics.json", "c3_english.json"].forEach((f) => { JSON.parse(fs.readFileSync(path.join(bd, f), "utf8")).forEach((q) => incoming.push(q)); });
["p4_analytical.json", "p4_mix.json"].forEach((f) => { JSON.parse(fs.readFileSync(path.join(bd, f), "utf8")).forEach((q) => { q.predicted = true; q.year = 2569; incoming.push(q); }); });
...
/* ---- 2) คลังเฉพาะทาง (เดิม + เติมแบบหลาย bank) ---- */
const META = {
...
/* ไฟล์เติมแบบหลาย bank (route ตาม field q.bank ของแต่ละข้อ) */
const appendByBank = {};
["xb_core_laws.json", "xb_other_laws.json", "xc_sjt.json"].forEach((f) => {
  JSON.parse(fs.readFileSync(path.join(bd, f), "utf8")).forEach((q) => {
    const b = q.bank;
    if (!META[b]) { issues.push("unknown bank " + b + " ใน " + f); return; }
    (appendByBank[b] = appendByBank[b] || []).push(q);
  });
});
...
let gid = core.length;
ORDER.forEach((bank) => {
  const raw = (banksExisting[bank] || []).concat(appendByBank[bank] || []);
  ...
  raw.forEach((q) => {
    ...
    if (META[bank].part === "ข") item.freq = FREQ_BANK[bank] || 3;
    list.push(item);
  });
...
const out = `/* ============================================================
   ...
   สร้างอัตโนมัติจาก _batches/assemble5.js
   ============================================================ */
